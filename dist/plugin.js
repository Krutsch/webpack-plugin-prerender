    ...
    return files4PR
        ...
        if (file + ".js" in assets) {
            arr.push({
                ...
                css: file + ".css" in assets ? file + ".css" : "",
            });
        }
        return arr;
    }, [])
...
class PRPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.emit.tapAsync("renderedHTML", async (compilation, callback) => {
            if (!isFirstRun) {
            ...
            renderedHTML = await preRender(html, files, options.externals, options.browser);
            callback();
        });
        compiler.hooks.done.tapAsync("reCompile", async (stats, callback) => {
            // start re-compilation
            compiler.compile((_, compilation) => {
